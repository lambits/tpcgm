...
if CLIENT then
    ...
    print("TPCGM Camera Initialized!")
    CreateConVar("tpc_enabled", 1, {FCVAR_ARCHIVE, FCVAR_NEVER_AS_STRING}, "Should the third person camera be enabled?", 0, 1)
    CreateConVar("tpc_smoothing", 1, {FCVAR_ARCHIVE, FCVAR_NEVER_AS_STRING}, "Should smoothing of the cameras be enabled? This does not apply to FOV.", 0, 1)
    CreateConVar("tpc_crouchoffset", 1, {FCVAR_ARCHIVE, FCVAR_NEVER_AS_STRING}, "Should crouching have an additional offset?", 0, 1)
    CreateConVar("tpc_fovmp", 1, {FCVAR_ARCHIVE, FCVAR_NEVER_AS_STRING}, "Should FOV be affected by certain actions?", 0, 1)
    CreateConVar("tpc_ftmp", 25, {FCVAR_ARCHIVE, FCVAR_NEVER_AS_STRING}, "The amount of times the Frame Time will be multiplied by. The higher, the faster.", 0, 100)
    CreateConVar("tpc_fovftmp", 5, {FCVAR_ARCHIVE, FCVAR_NEVER_AS_STRING}, "The amount of times the Frame Time will be multiplied by, ONLY for the FOV! The higher, the faster.", 0)
    CreateConVar("tpc_backoffset", 50, {FCVAR_ARCHIVE, FCVAR_NEVER_AS_STRING}, "How far should the camera be away from the player from back to front.", 0, 250)
    CreateConVar("tpc_rightoffset", 25, {FCVAR_ARCHIVE, FCVAR_NEVER_AS_STRING}, "How far should the camera be away from the player from left to right.", -100, 100)
    CreateConVar("tpc_upoffset", 5, {FCVAR_ARCHIVE, FCVAR_NEVER_AS_STRING}, "How far should the camera be away from the player from north to south.", -100, 100)
    CreateConVar("tpc_camtilt", 1, {FCVAR_ARCHIVE, FCVAR_NEVER_AS_STRING}, "The multiplier of how much camera tilt should be when falling.", 0, 100)
    CreateConVar("tpc_precision", 0, {FCVAR_ARCHIVE, FCVAR_NEVER_AS_STRING}, "Should the crosshair of the weapon the player is holding always be centered?", 0, 1)
    CreateConVar("tpc_deathcam", 0, {FCVAR_ARCHIVE, FCVAR_NEVER_AS_STRING}, "Should the camera be enabled while dead?", 0, 1)
    CreateConVar("tpc_fortnite", 15, {FCVAR_ARCHIVE, FCVAR_NEVER_AS_STRING}, "How much to the front should the camera be? 15 is recommended for battle royale.")
    CreateConVar("tpc_pitchrotation", 0, {FCVAR_ARCHIVE, FCVAR_NEVER_AS_STRING}, "How much should the camera be rotated from north to south. This value will NOT work when precision is enabled!", 0, 360)
    CreateConVar("tpc_yawrotation", 0, {FCVAR_ARCHIVE, FCVAR_NEVER_AS_STRING}, "How much should the camera be rotated from left to right. This value will NOT work when precision is enabled!", 0, 360)
    CreateConVar("tpc_rollrotation", 0, {FCVAR_ARCHIVE, FCVAR_NEVER_AS_STRING}, "How much should the camera be tilted. This value will NOT work when precision is enabled!", 0, 360)
    CreateConVar("tpc_drawviewer", 1, {FCVAR_ARCHIVE, FCVAR_NEVER_AS_STRING}, "Should the player be visible?", 0, 1)
    CreateConVar("tpc_togglekey", 0, {FCVAR_ARCHIVE, FCVAR_NEVER_AS_STRING}, "What key will toggle between first and third person?")
